...
function amb.registerServerCallback(callbackName, handler)
    ...
    storeCallbackHandler(callbackName, handler, 'server')
...
function amb.triggerClientCallback(callbackName, playerId, options, responseHandler, ...)
    ...
    serverCallbacks.pendingRequests[requestId] = requestData
    storePendingRequest(requestId, requestData, 'server')
    ...
    SetTimeout(callOptions.timeout, function()
        local pendingRequest = getPendingRequest(requestId, 'server')

        if pendingRequest then
            removePendingRequest(requestId, 'server')
...
RegisterNetEvent(SERVER_EVENTS.INCOMING_CALL, function(callbackName, requestingResource, requestId, ...)
    ...
    local handler = getCallbackHandler(callbackName, 'server')
